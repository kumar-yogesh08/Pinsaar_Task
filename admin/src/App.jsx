...
export default function App() {
  ...
  const onSubmit = async (v) => {
    ...
    reset({ title:'', body:'', releaseAt: dayjs().toISOString(), webhookUrl: v.webhookUrl });
    await load();
  };
    ...
    <div style={{ padding: 20, fontFamily: 'system-ui, sans-serif', maxWidth: 900, margin: '0 auto' }}>
      ...
      <section style={{ marginBottom: 24 }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'grid', gap: 8 }}>
          ...
          <textarea placeholder="Body" rows={4} {...register('body', { required: true })}/>
          <input placeholder="ReleaseAt ISO" {...register('releaseAt', { required: true })}/>
      ...
      <section>
        ...
        <div style={{ display:'grid', gap:6 }}>
          {notes.map(n => (
            <motion.div key={n.id}
              animate={n.status === 'delivered' ? { backgroundColor: ['#fff','#e6ffed','#fff'] } : {}}
